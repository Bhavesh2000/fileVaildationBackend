    ...
    data?.map((record, index) => {
      if (record != null && record != "NA") {
        if (index == 0) {
          ...
          let firstNameResonse = validator.isFirstNameValid(record,firstNameValidator);
          ...
          let lastNameResponse = validator.isLastNameValid(record, lastNameValidator);
          ...
          let maritalStatusResponse = validator.isMaritalStatusCorrect(record,maritalStatusValidator);
          ...
          let genderResponse = validator.isGenderStatusCorrect(record,genderStatusValidator);
          if (!genderResponse.isValid) {
            ...
            let contactNoResponse = validator.isContactNoNValid(record,contactNoValidator);
            ...
            let jobTitleResponse = validator.isJobTitleValid(record,jobTitleValidator);
  ...
  try {
    ...
    inValidRecords?.forEach(async (inValidRecord) => {
      let inValidRecordData = await InValidData.create({
        ...
        errorMessages:inValidRecord[7] == null ? "" : inValidRecord[7],
        fileBelong: fileId
      });
    });
  } catch (e) {
...
module.exports.validateFile = async function (req, res) {
  try {
    ...
    res.status(200).json({ message: "Parsed successfully", data: result, fileId:fildID });
  } catch (error) {
    res
      .status(500)
...
module.exports.getValidData = async function (req, res) {
  try {
    ...
    res.status(200).json({ message: "fetched summary data", data: validData });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error parsing File", error: error.message });
...
module.exports.getInValidData = async function (req, res) {
  try {
    ...
    res.status(200).json({ message: "fetched summary data", data: inValidData });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error parsing File", error: error.message });
...
module.exports.getSummaryDataByFileId = async function (req, res) {
  try {
    ...
    const response = await summaryData._doc;
    // if (!response.length) {
    //   return res.status(404).json({ message: "No data found for this fileId" });
    // }
    res.status(200).json({ message: "fetched valid data", data: response });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error parsing File", error: error.message });
